...
$defaultJsonString = Get-Content -Path $defaultJsonPath -Raw -Encoding UTF8
...
foreach ($htmFile in $htmFiles) {
    ...
    foreach ($jsonFile in $jsonFiles) {
        # Lese den Inhalt der aktuellen JSON-Datei UTF8
        $jsonContent = Get-Content -Path $jsonFile.FullName -Raw -Encoding UTF8
        # BOM am Anfang des Snippets entfernen (falls vorhanden)
        if ($jsonContent.Length -gt 0 -and $jsonContent[0] -eq [char]0xFEFF) {
            $jsonContent = $jsonContent.Substring(1)
        }

        # Lese den Inhalt der aktuellen HTM-Datei UTF8
        $htmContent = Get-Content -Path $htmFile.FullName -Raw -Encoding UTF8
        ...
        Set-Content -Path $newFilePath -Value $modifiedHtm -Encoding utf8
